...
| Correct teaching sail loop | `src/sim/model.ts`, `src/game/lesson.ts` | luff/attached/stall/no-go/tack tests plus keyboard/touch browser flows |
...
| Browser and app-ready architecture | static Vite/WebGL build, responsive DOM controls, `firebase.json` | local/live Chrome lanes; native wrappers deliberately deferred |
